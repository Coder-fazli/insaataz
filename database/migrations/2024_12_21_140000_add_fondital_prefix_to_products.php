...
return new class extends Migration
{
    public function up()
    {
        ...
        $categoryId = 96; // Dekorativ Radiatorlar (actual category ID)
    ...
    {
        // Remove Fondital prefix
        $categoryId = 96;
        $products = DB::table('products')
            ->where('category_id', $categoryId)
            ->whereRaw("JSON_EXTRACT(title, '$.az') LIKE 'Fondital %'")
            ->get();
        ...
        foreach ($products as $product) {
            ...
            foreach (['az', 'en', 'ru'] as $lang) {
                if (isset($title[$lang]) && stripos($title[$lang], 'Fondital ') === 0) {
                    $title[$lang] = substr($title[$lang], 9);
                }
            }
            DB::table('products')
                ->where('id', $product->id)
